    ...
    <div class="bg-white rounded-xl p-5 border border-gray-100">
        ...
        <div class="space-y-3">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <div class="h-2 bg-blue-100 rounded-full overflow-hidden w-24">
                        <div class="h-full bg-blue-400 rounded-full" style="width:<?= $clientStats['total'] > 0 ? ($clientStats['leads']/$clientStats['total']*100) : 0 ?>%"></div>
                    </div>
                    <span class="text-sm font-medium text-gray-700 w-6 text-right"><?= $clientStats['leads'] ?></span>
                </div>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-2">
                    <div class="h-2 bg-emerald-100 rounded-full overflow-hidden w-24">
                        <div class="h-full bg-emerald-400 rounded-full" style="width:<?= $clientStats['total'] > 0 ? ($clientStats['active']/$clientStats['total']*100) : 0 ?>%"></div>
                    </div>
                    <span class="text-sm font-medium text-gray-700 w-6 text-right"><?= $clientStats['active'] ?></span>
                </div>
            </div>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-2">
                    <div class="h-2 bg-gray-100 rounded-full overflow-hidden w-24">
                        <div class="h-full bg-gray-400 rounded-full" style="width:<?= $clientStats['total'] > 0 ? ($clientStats['completed']/$clientStats['total']*100) : 0 ?>%"></div>
                    </div>
                    <span class="text-sm font-medium text-gray-700 w-6 text-right"><?= $clientStats['completed'] ?></span>
                </div>
            </div>
        </div>
...
<div class="bg-white rounded-xl p-5 border border-gray-100">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <?php
        $actions = [
            ['label' => 'Add User',    'href' => 'admin/users/create',   'color' => 'indigo',  'icon' => 'M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z'],
        ...
        <a href="<?= BASE_URL ?>/<?= $action['href'] ?>"
           class="flex flex-col items-center gap-2 p-4 rounded-xl <?= $cls ?> transition-colors text-center group">
